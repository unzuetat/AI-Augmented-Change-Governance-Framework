...
export default function ChangeRegister() {
  ...
  const riskMap: Record<string, string> = {
    low: t.risk.low, medium: t.risk.medium, high: t.risk.high, critical: t.risk.critical,
  };

  const scopeLabelMap: Record<string, string> = {
    'project-only': t.register.project,
    'operational-only': t.register.ops,
    'cross-domain': t.register.cross,
  };
      ...
      <div className="bg-white rounded border border-gray-200 p-3">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div className="flex items-center gap-1 text-[10px] text-gray-400">
            <Filter size={11} />
            <span>{filtered.length} {t.register.of} {changes.length}</span>
          </div>
        </div>

        <div className="flex flex-wrap gap-2">
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="text-[10px] border border-gray-200 rounded px-2 py-1 text-gray-600 focus:outline-none focus:ring-2 focus:ring-cf-300">
            <option value="all">{t.register.allStatuses}</option>
            {uniqueStatuses.map((s) => (
              <option key={s} value={s}>{s.replace(/-/g, ' ')}</option>
            ))}
          </select>
          <select value={filterRisk} onChange={(e) => setFilterRisk(e.target.value)} className="text-[10px] border border-gray-200 rounded px-2 py-1 text-gray-600 focus:outline-none focus:ring-2 focus:ring-cf-300">
            <option value="all">{t.register.allRiskLevels}</option>
            <option value="low">{t.risk.low}</option>
            <option value="medium">{t.risk.medium}</option>
            <option value="high">{t.risk.high}</option>
            <option value="critical">{t.risk.critical}</option>
          </select>
          <select value={filterScope} onChange={(e) => setFilterScope(e.target.value)} className="text-[10px] border border-gray-200 rounded px-2 py-1 text-gray-600 focus:outline-none focus:ring-2 focus:ring-cf-300">
            <option value="all">{t.register.allScopes}</option>
          ...
          <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="text-[10px] border border-gray-200 rounded px-2 py-1 text-gray-600 focus:outline-none focus:ring-2 focus:ring-cf-300">
            <option value="all">{t.register.allTypes}</option>
      ...
      <div className="bg-white rounded border border-gray-200 overflow-hidden">
        <div className="grid grid-cols-[4rem_1fr_5rem_5rem_6rem_5.5rem] gap-2 px-4 py-2 border-b border-gray-200 bg-gray-50">
          <button onClick={() => toggleSort('id')} className="text-[10px] font-semibold text-gray-500 uppercase text-left hover:text-gray-700">
            {t.register.id} <SortIcon field="id" />
          </button>
          <span className="text-[10px] font-semibold text-gray-500 uppercase">{t.register.title2}</span>
          <span className="text-[10px] font-semibold text-gray-500 uppercase text-center">{t.register.type}</span>
          <button onClick={() => toggleSort('risk')} className="text-[10px] font-semibold text-gray-500 uppercase text-center hover:text-gray-700">
            {t.register.risk} <SortIcon field="risk" />
          </button>
          <span className="text-[10px] font-semibold text-gray-500 uppercase text-center">{t.register.scope}</span>
          <button onClick={() => toggleSort('status')} className="text-[10px] font-semibold text-gray-500 uppercase text-center hover:text-gray-700">
            {t.register.status} <SortIcon field="status" />
          </button>
        </div>
                ...
                >
                  ...
                  <div className="flex justify-center">
                    <span className={`text-[9px] font-semibold px-2 py-0.5 rounded-sm flex items-center gap-1 ${scopeColorMap[change.scope] || ''}`}>
                      ...
                      {scopeLabelMap[change.scope] || change.scope}
                    </span>
                  </div>
                  <div className="flex justify-center">
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 pt-1 bg-gray-50">
                    <div className="grid grid-cols-2 gap-4 text-xs">
                      <div>
                        <p className="text-gray-400 font-medium mb-1">{t.register.description}</p>
                        <p className="text-gray-600">{change.description}</p>
                      </div>
                      <div className="space-y-1.5">
                        <div>
                          <span className="text-gray-400 font-medium">{t.register.origin}: </span>
                          <span className="text-gray-600">{change.origin.type} — {change.origin.referenceName}</span>
                        </div>
                        <div>
                          <span className="text-gray-400 font-medium">{t.register.requested}: </span>
                          <span className="text-gray-600">{change.requestDate}</span>
                        </div>
                        <div>
                          <span className="text-gray-400 font-medium">{t.register.category}: </span>
                          <span className="text-gray-600">{change.category}</span>
                        </div>
                        <div>
                          <span className="text-gray-400 font-medium">{t.register.affectedServices}: </span>
                          <span className="text-gray-600">{change.affectedServices.length > 0 ? change.affectedServices.join(', ') : t.register.none}</span>
                        </div>
                        <div>
                          <span className="text-gray-400 font-medium">{t.register.affectedProjects}: </span>
                          <span className="text-gray-600">{change.affectedProjects.length > 0 ? change.affectedProjects.join(', ') : t.register.none}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {change.timeline.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-gray-200">
                        <p className="text-gray-400 font-medium text-xs mb-2">{t.register.timeline}</p>
                        <div className="space-y-1.5">
